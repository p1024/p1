...
async function getSongInfo(songId) {
	...
	let [response, body] = await request.getAsync({
		uri,
		headers: {
			/* 2018/2/21 22:48:30 require referer when request */
			Referer:`http://www.xiami.com/play?ids=/song/playlist/id/${songId}/object_name/default/object_id/0`
		}
	});
	if(response.statusCode !== 200) {
		throw Error({code: response.statusCode, status: response.statusMessage});
	} else {
		let trackList = eval(body).data.trackList;
		if(Array.isArray(trackList)) {
			song = trackList[0];
			song.location = getLocation(song.location);
		} else {
			throw new Error('song not exist cuz copyright');
		}
	}
